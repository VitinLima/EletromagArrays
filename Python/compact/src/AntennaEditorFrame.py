...
class AntennaEditorFrame(tk.Frame):
    ...
    def init_variables(self,):
        ...
        self.ideal_dipole_length = tk.DoubleVar(
            value=self.antenna.evaluation_arguments['dipole length'])
        ...
        self.expression_phi = tk.StringVar(
            value=self.antenna.evaluation_arguments['expression phi'])
        self.isotropic_radiobutton_variable = tk.StringVar(
            value=self.antenna.evaluation_arguments['isotropic on'])
        self.file_path = tk.StringVar(
            value=self.antenna.evaluation_arguments['file path'])
    ...
    def init_layout(self):
        ...
        fr = ttk.LabelFrame(
            master=self,
            text='Phi angle samples: linspace(start, stop, num)')
        ...
        fr = ttk.LabelFrame(
            master=self,
            text='Theta angle samples: linspace(start, stop, num)')
        fr.pack(side='top', fill='both')
        ttk.Label(master=fr, text='Start').grid(row=0, column=0)
        ttk.Label(master=fr, text='Stop').grid(row=0, column=1)
        ttk.Label(master=fr, text='Num').grid(row=0, column=2)
        ...
        tk.Radiobutton(
            master=radiobutton_frame, text='Ideal dipole',
            value='Ideal dipole',
            variable=self.radiobutton_variable,
            command=self.radiobutton_change
            ).pack(side=tk.TOP, fill=tk.BOTH)
        tk.Radiobutton(
            master=radiobutton_frame, text='Ideal loop dipole',
            value='Ideal loop dipole',
            variable=self.radiobutton_variable,
            command=self.radiobutton_change
            ).pack(side=tk.TOP, fill=tk.BOTH)
        ...
        tk.Button(master=radiobutton_frame, text='Load from file...',
        ...
        self.isotropic_frame = tk.LabelFrame(
            master=electric_field_frame, text='Isotropic antenna')
        tk.Radiobutton(
            master=self.isotropic_frame, text='Isotropic on both',
            value='both',
            variable=self.isotropic_radiobutton_variable).pack(
                side=tk.TOP, fill=tk.BOTH)
        tk.Radiobutton(
            master=self.isotropic_frame, text='Isotropic on theta',
            value='theta',
            variable=self.isotropic_radiobutton_variable).pack(
                side=tk.TOP, fill=tk.BOTH)
        tk.Radiobutton(
            master=self.isotropic_frame,
            text='Isotropic on phi', value='phi',
            variable=self.isotropic_radiobutton_variable).pack(
                side=tk.TOP, fill=tk.BOTH)
        self.ideal_dipole_frame = tk.LabelFrame(
            master=electric_field_frame,
            text='Ideal dipole length (L)')
        self.ideal_loop_dipole_frame = tk.LabelFrame(
            master=electric_field_frame,
            text='Ideal loop dipole area (S)')
        self.expression_frame = tk.LabelFrame(
            master=electric_field_frame,text='Expressions')
        self.load_file_frame = tk.LabelFrame(
            master=electric_field_frame, text='Load file')
        ttk.Label(master=self.expression_frame,
                  text='Expression theta').grid(row=0, column=0)
        ttk.Label(master=self.expression_frame,
                  text='Expression phi').grid(row=1, column=0)
        ttk.Entry(
        ...
        tk.Checkbutton(
            master=self.load_file_frame, text='Load mesh from file',
    ...
    def _on_update(self):
        ...
        self.antenna.evaluation_arguments['dipole length'] = \
            self.ideal_dipole_length.get()
        self.antenna.evaluation_arguments['loop dipole area'] = \
            self.ideal_loop_dipole_area.get()
        self.antenna.evaluation_arguments['expression theta'] = \
            self.expression_theta.get()
        self.antenna.evaluation_arguments['expression phi'] = \
            self.expression_phi.get()
        self.antenna.evaluation_arguments['isotropic on'] = \
            self.isotropic_radiobutton_variable.get()
        self.antenna.evaluation_arguments['file path'] = \
            self.file_path.get()
        self.antenna.evaluation_arguments['force reload'] = True
        self.antenna.evaluation_arguments['load mesh from file'] = \
            self.load_mesh_from_file.get()

        self.antenna.set_current(
            current_magnitude=self.current_magnitude.get(),
