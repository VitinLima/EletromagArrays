...
def cost_function(x, target, array):
    ...
    if len(array.antennas) & 1:
        ...
        # k = 1
        # for i in range(1,N+1):
        #     array.antennas[N + k].set_orientation(
        #         elevation=-90*x[N+i])
        #     array.antennas[N - k].set_orientation(
        #         elevation=-90*(1-x[N+i]))
        #     k = k+1
    else:
        N = int(len(array.antennas)/2)
        k = 0
        ...
        #     array.antennas[N + k].set_orientation(
        #         elevation=-90*x[N+i])
        #     array.antennas[N-1 - k].set_orientation(
        #         elevation=-90*(1-x[N+i]))
        #     k = k+1
...
# k = -0.1
for i in range(len(initial_array.antennas)):
    initial_array.antennas[i].set_position(
        # x=2*(np.random.rand()-0.5)*len(initial_array.antennas),
        # y=2*(np.random.rand()-0.5)*len(initial_array.antennas),
        x = 0,
        y = i*0.5,
        z=0)
    initial_array.antennas[i].set_orientation(
        elevation=-90,
        azimuth=0,
        roll=90*(np.cos(np.pi*i)/2+0.5))
    initial_array.antennas[i].set_current(
        magnitude = 1/(2*i+1),
...
if N & 1:
    ...
    #     ]
    x0 = [initial_array.antennas[i].current_magnitude
