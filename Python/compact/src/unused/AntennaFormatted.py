...
class Antenna:
    ...
    def __init__(self, 
                 constants=None, name='new antenna',
                 current_magnitude=1, current_phase=0,
    ...
    def evaluate_local_mesh(self):
        ...
        self.local_shape = (self.local_theta.size,
                            self.local_phi.size)
        ...
        self.local_hat_k = np.matmul(
            R, self.local_hat_k[:, :, :, np.newaxis]).squeeze()
        self.local_hat_theta = np.matmul(
            R, self.local_hat_theta[:, :, :, np.newaxis]).squeeze()
        self.local_hat_phi = np.matmul(
            R, self.local_hat_phi[:, :, :, np.newaxis]).squeeze()
    ...
    def evaluate_LG_interp_mesh(self):
        ...
        self.LG_interp_hat_theta = MyMath.rotate(MyMath.rotate(
            MyMath.rotate(L_interp_hat_theta, Rtheta), Rphi), LG_R)
        self.LG_interp_hat_phi = MyMath.rotate(MyMath.rotate(
            MyMath.rotate(L_interp_hat_phi, Rtheta), Rphi), LG_R)
    ...
    def get_reference_polarization(self):
        ...
        hat_i_ref_z = - st*sp
        hat_i_ref = np.array([hat_i_ref_x, hat_i_ref_y, hat_i_ref_z]).swapaxes(
            0, 1).swapaxes(1, 2)

        hat_i_cross_x = (1 - cp*cp*(1-ct))
        hat_i_cross_y = - (1-ct)*sp*cp
        hat_i_cross_z = - st*cp
        hat_i_cross = np.array(
            [hat_i_cross_x, hat_i_cross_y, hat_i_cross_z]).swapaxes(0, 1).swapaxes(1, 2)
    ...
    def evaluate_local_field(self):
        ...
        if self.evaluate_as == 'ideal loop dipole':
            ...
            self.eval_expression(
                self.evaluation_arguments['expression theta'], self.evaluation_arguments['expression phi'])
            # else:
            #     raise Exception('Tried to evaluate expression without expression.')
        # except Exception as e:
        #     print(e)

        # self.local_Fphi *= self.current_magnitude*np.exp(1j*np.radians(self.current_phase))
        # self.local_Ftheta *= self.current_magnitude*np.exp(1j*np.radians(self.current_phase))
    ...
    def evaluate(self):
        ...
        interp_points = (self.GL_interp_mesh_theta,
        ...
        self.Ftheta = Ftheta*(self.LG_interp_hat_theta*self.hat_theta).sum(
            2) + Fphi*(self.LG_interp_hat_phi*self.hat_theta).sum(2)
        self.Fphi = Ftheta*(self.LG_interp_hat_theta*self.hat_phi).sum(
            2) + Fphi*(self.LG_interp_hat_phi*self.hat_phi).sum(2)
        ...
        vector_F = self.Ftheta[:, :, np.newaxis] * \
            self.hat_theta + self.Fphi[:, :, np.newaxis]*self.hat_phi
        self.Fx = vector_F[:, :, 0]
        self.Fy = vector_F[:, :, 1]
        self.Fz = vector_F[:, :, 2]
    ...
    def ideal_dipole(self, L):
        ...
        self.local_Ftheta[ids] = (
            np.cos(A*np.cos(self.local_mesh_theta[ids])) - np.cos(A))/st[ids]
    ...
    def load_file(self, file_path):
        ...
        with open(file_path, 'r') as f:
            ...
            Fphi = 4*np.pi*np.array(rEphi) / \
                (1j*constants['eta']*constants['k'])
            Ftheta = 4*np.pi * \
                np.array(rEtheta)/(1j*constants['eta']*constants['k'])
            ...
            if self.evaluation_arguments['load mesh from file']:
                ...
                self.local_shape = (
                    self.local_theta.size, self.local_phi.size)
                ...
                interp_points = (
                    self.local_mesh_theta, self.local_mesh_phi)
...
class Antenna:
    ...
    def __init__(
            self, constants=None, name='new antenna',
            current_magnitude=1, current_phase=0, theta=np.linspace(
                0, 180, 21),
            phi=np.linspace(-180, 180, 21),
            local_theta_deg=np.linspace(0, 180, 91),
            local_phi_deg=np.linspace(-180, 180, 91),
            roll=0, elevation=0, azimuth=0, x=0, y=0, z=0,
            evaluate_as='ideal dipole'):
        # self.constants=constants
        self.name = name
        self.current_magnitude = current_magnitude
    ...
    def get_reference_polarization(self):
        ...
        hat_i_ref_z = - st*sp
        hat_i_ref = np.array(
            [hat_i_ref_x, hat_i_ref_y, hat_i_ref_z]).swapaxes(
            0, 1).swapaxes(
            1, 2)
        ...
        hat_i_cross_z = - st*cp
        hat_i_cross = np.array(
            [hat_i_cross_x, hat_i_cross_y, hat_i_cross_z]).swapaxes(
            0, 1).swapaxes(
            1, 2)
    ...
    def interpolate_at(self, interp_mesh_theta_deg,
                       interp_mesh_phi_deg, field):
        fit_points = (self.theta, self.phi)
        interp_points = (interp_mesh_theta_deg, interp_mesh_phi_deg)
    ...
    def copy(self):
        antenna = Antenna(
            constants=constants, name=self.name,
            current_magnitude=self.current_magnitude,
            current_phase=self.current_phase, phi=self.phi.copy(),
            theta=self.theta.copy(),
            elevation=self.elevation, azimuth=self.azimuth,
            roll=self.roll, x=self.x, y=self.y, z=self.z,
            evaluate_as=self.evaluate_as)
...
class Antenna:
    ...
    def evaluate_LG_interp_mesh(self):
        ...
        # G - Global
        # GL - Global on Local for vectors or Global to Local for matrices
        # LG - Local on Global for vectors or Local to Global for
        # matrices
    ...
    def evaluate_local_field(self):
        ...
        if self.evaluate_as == 'isotropic':
            ...
            #     raise Exception('Tried to calculate ideal dipole field without dipole length.')
        if self.evaluate_as == 'ideal loop dipole':
            # if 'loop dipole area' in
            # self.evaluation_arguments.keys():
            self.ideal_loop_dipole(
                self.evaluation_arguments['loop dipole area'])
            # else:
            #     raise Exception('Tried to calculate ideal dipole field without dipole length.')
            ...
            # if 'expression theta' in
            # self.evaluation_arguments.keys() and 'expression phi' in
            # self.evaluation_arguments.keys():
            self.eval_expression(
                self.evaluation_arguments['expression theta'],
                self.evaluation_arguments['expression phi'])
    ...
    def evaluate(self):
        ...
        vector_F = self.Ftheta[:,
                               :,
                               np.newaxis] * self.hat_theta + self.Fphi[:,
                                                                        :,
                                                                        np.newaxis] * self.hat_phi
        self.Fx = vector_F[:, :, 0]
        self.Fy = vector_F[:, :, 1]
        self.Fz = vector_F[:, :, 2]
    ...
    def load_file(self, file_path):
        ...
        with open(file_path, 'r') as f:
            ...
            Fphi = 4 * np.pi * np.array(rEphi) / \
                (1j * constants['eta'] * constants['k'])
            Ftheta = 4 * np.pi * np.array(rEtheta) / (1j *
                                                      constants['eta'] * constants['k'])
            idsphi = theta == theta[0]
            idstheta = phi == phi[0]
            phi = phi[idsphi]
            theta = theta[idstheta]
