...
class Antenna:
    ...
    def evaluate_local_field(self):
        ...
        if self.evaluate_as == 'isotropic':
            ...
            if 'dipole length' in self.evaluation_arguments.keys():
                ...
                raise Exception('Tried to calculate ideal dipole ' +
        ...
        if self.evaluate_as == 'ideal loop dipole':
            ...
            if 'file path' in self.evaluation_arguments.keys():
                ...
                raise Exception('Tried to load file ' +
                                'without file path.')
            ...
            if 'expression theta' \
                ...
                raise Exception('Tried to evaluate expression ' +
                                'without expression.')
    ...
    def evaluate(self):
        ...
        self.Ftheta = Ftheta*(
            self.LG_interp_hat_theta*self.hat_theta).sum(
            2) + Fphi*(self.LG_interp_hat_phi*self.hat_theta).sum(2)
        self.Fphi = Ftheta*(
            self.LG_interp_hat_theta*self.hat_phi).sum(
    ...
    def ideal_dipole(self, L):
        ...
        self.local_Ftheta[ids] = (
            np.cos(A*np.cos(self.local_mesh_theta[ids])) -
            np.cos(A))/st[ids]
