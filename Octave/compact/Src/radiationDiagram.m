function radiationDiagram(antenna, beta, alpha, showIm)
  ...
  interp_theta = atan2d(sqrt(dot(
    points(1:2,:),points(1:2,:),1)),points(3,:));

  antenna = toMeshShape(antenna);
  Ephi = interp2(
    antenna.THETA_samplings,
    antenna.PHI_samplings,
    antenna.Ephi, interp_theta, interp_phi);
  Etheta = interp2(
    antenna.THETA_samplings,
    antenna.PHI_samplings,
    antenna.Etheta, interp_theta, interp_phi);
