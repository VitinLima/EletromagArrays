function ant = rotateElectricFields(ant, alpha, beta)
  ...
  tool_ant.THETA = atan2d(sqrt(dot(tool_ant.k_hat(1:2,:), ...
    tool_ant.k_hat(1:2,:), 1)), tool_ant.k_hat(3,:));
  ...
  tool_ant.Etheta = interp2(ant.THETA, ant.PHI, ant.Etheta, ...
    tool_ant.THETA, tool_ant.PHI);
  tool_ant.Ephi = interp2(ant.THETA, ant.PHI, ant.Ephi, ...
    tool_ant.THETA, tool_ant.PHI);

  ant = toVectorShape(ant);
  tool_ant = toVectorShape(tool_ant);

  rE_vec = tool_ant.theta_hat.*tool_ant.Etheta + ...
    tool_ant.phi_hat.*tool_ant.Ephi;
