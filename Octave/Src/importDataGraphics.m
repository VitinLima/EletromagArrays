...
function invertedSphere_rE(varargin)
  ...
  for i = 1:length(Theta_ticks)
    ...
    R = R_ticks(end);
    thick_handles(end+1) = text(R*cosd(Theta_ticks(i)), R*sind(Theta_ticks(i)), [num2str(Theta_ticks(i)),"^\\circ"], 'verticalalignment', v_alignment, 'horizontalalignment', h_alignment);
  end
