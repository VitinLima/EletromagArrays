...
##  ant = toVectorShape(ant);
  quiver3(
    ant.k_hat(1,:),
    ant.k_hat(2,:),
    ant.k_hat(3,:), field(1,:), field(2,:), field(3,:));
...
function invertedSphere_rE(varargin)
  ...
  figure('visible', args_in.showIm, ...
    'filename', ['inverted sphere rE', args_in.title]);
  hold on;
  ...
  for i = 1:length(R_ticks)
    ...
    thick_handles(end+1) = text(R*cosd(67.5), R*sind(67.5), ...
      [num2str(R_ticks(i)),"º"], ...
      'horizontalalignment', 'center', ...
      'verticalalignment', 'bottom');
  end
  for i = 1:length(Theta_ticks)
    grid_handles(end+1) = polar(deg2rad(Theta_ticks(i))*[1 1],
      [R_ticks(1) R_ticks(end)]);
    ...
    R = R_ticks(end);
    thick_handles(end+1) = text(R*cosd(Theta_ticks(i)), ...
      R*sind(Theta_ticks(i)), [num2str(Theta_ticks(i)),"º"], ...
